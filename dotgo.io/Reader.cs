...
namespace dotgo.io
{
    ...
    ///  any error encountered.Even if Read returns n<len(p), it may use all of p as scratch space during the call.
    ///  If some data is available but not len(p) bytes, Read conventionally returns what is available instead of waiting for more.
    ///  When Read encounters an error or end-of-file condition after successfully reading n > 0 bytes, it returns
    ///  the number of bytes read.
    ///  It may return the(non-nil) error from the same call or return the error(and n == 0) from a subsequent call.
    ///  An instance of this general case is that a Reader returning a non-zero number of bytes at the end of the input stream 
